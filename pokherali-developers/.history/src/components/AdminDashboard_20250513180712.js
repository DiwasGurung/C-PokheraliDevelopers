...
import { PlusCircle, BookOpen, ShoppingBag, BarChart3, Filter } from "lucide-react";
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        ...
        // Use the correct parameter names expected by the BookFilterDto
        ...
        if (error.response) {
          ...
          setError(`Failed to load books: ${error.response.status}`);
    ...
    // Always attempt to fetch data - if session is invalid, the error will redirect
    fetchBooks();
    fetchOrders();
  }, []);
    ...
    try {
      ...
      if (error.response) {
        if (error.response.status === 401 || error.response.status === 403) {
          ...
          console.error("Error details:", error.response.data);
          alert(`Error adding book: ${error.response.status}`);
        }
      } else {
    ...
    try {
      ...
      if (error.response) {
        if (error.response.status === 401 || error.response.status === 403) {
          ...
          console.error("Error details:", error.response.data);
          alert(`Error updating book: ${error.response.status}`);
        }
      } else {
    ...
    try {
      ...
      if (error.response) {
        if (error.response.status === 401 || error.response.status === 403) {
          ...
          console.error("Error details:", error.response.data);
          alert(`Error deleting book: ${error.response.status}`);
        }
      } else {
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <h2 className="text-3xl font-bold text-gray-900 mt-1">
                  {Array.isArray(books) && books.filter((item) => item.stockQuantity < 20).length}
                </h2>
              </div>
