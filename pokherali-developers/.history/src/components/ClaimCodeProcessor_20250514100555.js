...
export default function ClaimCodeProcessor() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorData = await response.text(); // Use text() instead of json() in case of empty body
      if (errorData) {
        const parsedError = JSON.parse(errorData); // Only parse if response contains valid JSON
        throw new Error(parsedError.message || "Invalid or used claim code");
      }
      throw new Error("Something went wrong, please try again later.");
    }

    const data = await response.json(); // Safely parse JSON only if the response is OK
    setSuccess(true);
    ...
    if (err.message === "Not authenticated. Please log in.") {
          ...
          <div className="mt-8 p-4 bg-blue-50 rounded-lg flex items-start">
            ...
            <div>
              ...
              <ol className="text-blue-600 text-sm list-decimal pl-4 space-y-1">
                ...
                <li>
                  Mark the order as "Delivered" in the admin dashboard after
