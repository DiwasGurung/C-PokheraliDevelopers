...
export default function Cart() {
  ...
  const [checkoutStep, setCheckoutStep] = useState(1); // 1: Cart, 2: Shipping, 3: Review, 4: Confirmation
  ...
  const handleShippingSubmit = (e) => {
    ...
    setError("");
    setCheckoutStep(3); // Move to review step
  };
    ...
    return (
      <div>
        <div className="flex items-center mb-4">
          ...
          <h2 className="text-xl font-semibold ml-4">Review Your Order</h2>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="space-y-4">
            {cartItems.map((item) => (
              <div key={item.id} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-gray-900">{item.bookTitle || "Untitled Book"}</h4>
                  <p className="text-sm text-gray-500">
                    {item.quantity} × ${(item.bookPrice || 0).toFixed(2)}
                  </p>
                </div>
                
                <div className="text-right">
                  <p className="font-medium">${((item.bookPrice || 0) * (item.quantity || 0)).toFixed(2)}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <button 
            onClick={() => setCheckoutStep(1)} 
            className="text-purple-600 text-sm font-medium mt-3 hover:text-purple-800"
          >
            Edit Cart
          </button>
          ...
          <div className="flex">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                By placing this order, you agree to our Terms and Conditions. Your books will be available for pickup at our store within 24 hours of order confirmation.
              </p>
            </div>
          </div>
        </div>
  ...
  const renderConfirmation = () => {
    if (!orderDetails && !orderComplete) return null;
    ...
    return (
      <div className="text-center">
        ...
        <div className="bg-gray-50 p-4 rounded-lg mb-6 text-left">
          ...
          <p><span className="text-gray-600">Total Amount:</span> ${(orderDetails?.totalAmount || totals.finalTotal).toFixed(2)}</p>
          ...
          {orderDetails?.claimCode && (
            <div className="mt-4">
              ...
              <div className="bg-white p-3 border border-gray-300 rounded-lg text-center">
                <span className="text-xl font-mono font-semibold tracking-widest">{orderDetails.claimCode}</span>
              </div>
