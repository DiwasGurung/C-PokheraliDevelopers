...
const BookDetails = () => {
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      await axios.post("https://localhost:7126/api/Reviews", { 
        bookId: parseInt(id), 
        comment: review,
        rating: rating // Include the rating in the request
      })
      ...
      if (error.response) {
        if (error.response.status === 400 && error.response.data === "You can only review books you have purchased") {
          setReviewMessage({ type: "error", text: "You can only review books you have purchased." })
        } else if (error.response.status === 400 && error.response.data === "You have already reviewed this book") {
          setReviewMessage({ type: "error", text: "You have already reviewed this book." })
        } else {
