...
const BookCatalog = () => {
  ...
  const fetchBooks = async () => {
    setLoading(true);
    try {
      // Convert filters object to match the expected BookFilterDto format
      const params = {
        ...
        awardWinner: filters.awardWinner,
  ...
  const handleFilterChange = (newFilters) => {
    // Map the filter component's output to match our expected format
    const mappedFilters = {
      ...filters,
      ...newFilters,
      // Set page to 1 when filters change (done in setPage below)
    };
