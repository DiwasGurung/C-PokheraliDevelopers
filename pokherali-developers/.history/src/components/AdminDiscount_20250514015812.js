...
export default function AdminDiscounts() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const discountStartDate = new Date(formData.startDate);
      const discountEndDate = new Date(formData.endDate);
      ...
      const updateData = {
        ...
        originalPrice: selectedBook.originalPrice || selectedBook.price
      };
      
      // Send the update request
      await axios.put(`https://localhost:7126/api/Books/${selectedBook.id}`, updateData, {
  ...
  const handleRemoveDiscount = async (book) => {
    ...
    try {
      // Prepare data to remove discount
      const updateData = {
        id: book.id,
        isOnSale: false,
      ...
      // Send the update request
      await axios.put(`https://localhost:7126/api/Books/${book.id}`, updateData, {
        headers: { 'Content-Type': 'application/json' },
        withCredentials: true
      });
      ...
      setError(err.response?.data?.message || err.message || 'Failed to remove discount');
