...
const BookCatalog = () => {
  ...
  // Fetch books with minimal required parameters
    ...
    try {
      const response = await axios.get('https://localhost:7126/api/Books', {
        params: {
          ...
          authors: [],  // Empty list to satisfy DTO requirements
          languages: [], // Empty list to satisfy DTO requirements
          formats: [],  // Empty list to satisfy DTO requirements
          publishers: [], // Empty list to satisfy DTO requirements
          sortBy: sortBy,
        ...
        paramsSerializer: params => {
          ...
          Object.entries(params).forEach(([key, value]) => {
            if (Array.isArray(value)) {
              // For array params, add each value separately or an empty string if empty
              if (value.length > 0) {
              ...
              // For non-array params, add if not null/undefined
