...
import { Heart, ShoppingCart, BookOpen, ChevronLeft, ChevronRight, CheckCircle, XCircle } from 'lucide-react';
...
const BookCatalog = () => {
  ...
  // Show notification for a specified duration
  ...
  const handleAddToCart = async (book) => {
    ...
    try {
      ...
      showNotification(`Failed to add ${book.title} to cart.`, "error");
    } finally {
  ...
  const handleBookmark = async (book) => {
    if (!user) {
      showNotification("Please log in to bookmark books.", "error");
      setTimeout(() => {
        navigate('/login', { state: { redirectTo: '/' } });
      }, 1500);
      return;
    ...
    try {
      if (bookmarked) {
        ...
        showNotification(`${book.title} removed from your bookmarks`);
      } else {
        ...
        showNotification(`${book.title} added to your bookmarks!`);
      }
    } catch (error) {
      console.error(`Error ${bookmarked ? 'removing' : 'adding'} bookmark:`, error);
      showNotification(`Failed to ${bookmarked ? 'remove' : 'add'} bookmark.`, "error");
    } finally {
            ...
            books.map((book) => (
              <div key={book.id} className="border rounded-lg overflow-hidden shadow-md bg-white transition-transform hover:shadow-lg hover:-translate-y-1">
                <div className="relative h-48">
                  ...
                  >
                    {processingActions[`bookmark-${book.id}`] ? (
                      <div className="h-[18px] w-[18px] rounded-full border-2 border-gray-400 border-t-transparent animate-spin"></div>
                    ) : (
