...
export default function Cart() {
  ...
  const processOrder = async () => {
    ...
    try {
      ...
      setOrderComplete(true);  // This ensures the order is marked as complete
      clearCart();
    ...
    if (!orderDetails || Object.keys(orderDetails).length === 0) {
      return (
        <div className="text-center">
          <p>Something went wrong. No order details were returned.</p>
          <button
            onClick={() => setCheckoutStep(1)} // Go back to cart or another page
